    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(_buckets):
            if inst_size < bucket_size:
                ...
                data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, stacked_heads, children, siblings, stacked_types, skip_connect, previous, next])
...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
	...
	next_inputs = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)	
        ...
        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, stack_hids, chids, ssids, stack_tids, skip_ids, previous_ids, next_ids = inst
            inst_size = len(wids)
